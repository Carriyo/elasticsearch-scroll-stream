...
class LibElasticsearchScrollStream extends Readable {
  ...
  getMoreUntilDone(err, response) {
    ...
    body.hits.hits.forEach(hit => {
      // With newer ES (2.4+) scripted fields and _source can both be present.
      // A work-around for now is to merge them together
      let ref_results = hit._source || {}
      if (hit.fields) {
        Object.assign(ref_results, hit.fields);
      }
